...
import { MessageSquare, ExternalLink, Loader2, CheckCircle, AlertCircle, XCircle } from 'lucide-react';
import { api } from '../api';

/**
 * Slack OAuth connection widget.
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 p-3 bg-dark-800/30 rounded-lg border border-dark-700/30">
        ...
        <span className="text-xs text-dark-400">Checking Slack status...</span>
      </div>
    );
  }
    ...
    return (
      <div className="p-3 bg-dark-800/30 rounded-lg border border-dark-700/30">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-sm font-medium text-dark-300">Slack</span>
    ...
    <div className={`p-3 rounded-lg border transition-colors ${
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <MessageSquare className={`w-4 h-4 ${status.connected ? 'text-emerald-400' : 'text-dark-400'}`} />
          <span className="text-sm font-medium text-dark-200">Slack</span>
          {status.connected ? (
            <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
              ...
              {(status as any).teamName || 'Connected'}
            </span>
          ) : (
        ...
        {status.connected ? (
          ...
          <button
            ...
            disabled={connecting}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#4A154B] hover:bg-[#5b1b5d] text-white rounded-lg text-xs font-medium transition-colors disabled:opacity-40"
          >
            {connecting ? (
