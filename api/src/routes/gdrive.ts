...
export function gdriveRoutes() {
  ...
  router.get('/auth-url', (req, res) => {
    const config = getGoogleOAuthConfig();
    if (!config) {
      return res.status(500).json({ error: 'Google OAuth not configured. Set GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_REDIRECT_URI (or the legacy GMAIL_*/GDRIVE_* equivalents) — the same OAuth client is reused for Gmail and Drive.' });
    }
