...
export async function analyzeAndCommentJira(jiraKey, task, agentId, agentManager, instructions, role) {
  ...
  let agent: any = null;
  if (role) {
    const agents = Array.from(agentManager.agents.values()) as any[];
    agent = agents.find(
      (a: any) => a.enabled !== false && a.status === 'idle' && (a.role || '').toLowerCase() === role.toLowerCase()
    );
  }
  if (!agent) {
    // Fallback: use any idle enabled agent (prefer leader)
    const agents = Array.from(agentManager.agents.values()) as any[];
    agent = agents.find((a: any) => a.enabled !== false && a.status === 'idle' && a.isLeader)
      || agents.find((a: any) => a.enabled !== false && a.status === 'idle');
  }
