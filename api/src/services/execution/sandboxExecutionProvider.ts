...
export class SandboxExecutionProvider extends ExecutionProvider {
  sandboxServiceFilter: string;
  baseWorkspace: string;
  agentUsers: Map<string, AgentUserEntry>;
  _resolvedContainerName: string | null;
  _fileTreeCache: Map<string, FileTreeCacheEntry>;
  _lastContainerCheck: number | null;
  ...
  async _generateFileTree(agentId: string): Promise<void> {
    const entry = this.agentUsers.get(agentId);
    if (!entry) return;
  ...
  async _switchProject(agentId: string, newProject: string | null, gitUrl: string | null = null): Promise<void> {
    const entry = this.agentUsers.get(agentId);
  ...
  async _ensureSharedContainerRunning(): Promise<void> {
  ...
  async _discoverContainer(): Promise<string> {
  ...
  async _ensureLinuxUser(username: string): Promise<void> {
  ...
  async _ensureAgentWorkspace(username: string): Promise<void> {
    const workspace = this._sh(this._userWorkspace(username));
    const userEsc = this._sh(username);
    await this._execAsRoot(`mkdir -p ${workspace} && chown -R ${userEsc}:${userEsc} ${workspace}`);
  }

  async _cloneProjectForUser(username: string, project: string, gitUrl: string): Promise<void> {
    const userEsc = this._sh(username);
  ...
  async _execPipedAsUser(username: string, innerCmd: string, stdinContent: string): Promise<{ stdout: string; stderr: string }> {
    const runPiped = (containerName: string): Promise<{ stdout: string; stderr: string }> => new Promise((resolve, reject) => {
      const proc = exec(
  ...
  async _execAsRoot(command: string, { timeout = 120000 }: { timeout?: number } = {}): Promise<{ stdout: string; stderr: string }> {
  ...
  async _execAsAgentUser(username: string, command: string, { cwd = null, timeout = 120000 }: { cwd?: string | null; timeout?: number } = {}): Promise<{ stdout: string; stderr: string }> {
  ...
  async _isRunning(containerName: string): Promise<boolean> {
    try {
  ...
  _username(agentId: string): string {
    const safe = String(agentId).toLowerCase().replace(/[^a-z0-9]/g, '');
    return `agent_${safe.slice(0, 24) || 'user'}`;
  }

  _userWorkspace(username: string): string {
    return `${this.baseWorkspace}/${username}`;
  }

  _projectPath(entry: AgentUserEntry, relativePath: string): string {
