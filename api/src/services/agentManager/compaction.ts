...
export const compactionMethods = {
   ...
   */
  _estimateTokens(this: any, messages: any[]): number {
    let chars = 0;
  ...
  _parseRateLimitReset(this: any, text: string): { retryAt: number; resetLabel: string } | null {
    ...
    const offsetMs = (utcDate as any) - (tzDate as any);
   ...
   */
  async _compactHistory(this: any, agent: any, keepRecent: number = 10): Promise<void> {
    ...
    const realHistory = history.filter((m: any) => m.type !== 'compaction-summary');
